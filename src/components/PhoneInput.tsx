...
export const PhoneInput: React.FC<PhoneInputProps> = ({
	...
	const formatPhoneNumber = (input: string, maxLength: number): string => {
		...
		if (maxLength === 10) {
			...
			if (limitedDigits.length <= 1) {
				...
				return `${limitedDigits.slice(0, 1)} (${limitedDigits.slice(
					1,
					4
				...
				return `${limitedDigits.slice(0, 1)} (${limitedDigits.slice(
					1,
					4
				)}) ${limitedDigits.slice(4, 7)}-${limitedDigits.slice(7)}`;
			}
		} else {
	...
	const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const phoneLength = selectedCountry
			? parseInt(selectedCountry.phone_length)
			: 10;
	...
	return (
		<div className='relative'>
			<input
				...
				className={`w-full py-3 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors pr-4 pl-4`}
			/>
		</div>
	);
};
